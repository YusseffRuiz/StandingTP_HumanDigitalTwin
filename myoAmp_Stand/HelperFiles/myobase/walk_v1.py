...
class WalkEnvV0(BaseV0):
    #TODO
    #Modify movement on motor foot. Compare with shadow hand environment to modify values.
    #Track movement of the natural foot and give rewards for similar movement.
    DEFAULT_OBS_KEYS = [
        "qpos_without_xy",
        "qvel",
        "com_vel",
    ...
    def _get_feet_heights(self):
        ...
        """
        foot_id_l = self.sim.model.body_name2id("l_Ankle_Assembly")
    ...
    def _get_pelvis_move(self):
        """
        Incentivize staying close to the initial position of pelvis traslation up to a certain threshold.
        """
        target_pos = [
            self.target_pos if self.target_pos is not None else self.init_qpos[0:2]
        ][0]
        return np.exp(-np.linalg.norm(5.0 * (self.sim.data.qpos[0:2] - target_pos)))
